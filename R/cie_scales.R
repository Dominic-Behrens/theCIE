#' Scale constructor for CIE colours
#'
...
#' \code{\link{make_cie_pal}}.
#'
#' @param palette Character name of palette in \code{cie_palettes}
...
#'   TRUE or FALSE
#'
#' @describeIn scale_colour_cie For colour scales
#'
#' @export
scale_colour_cie <- function(
  ...
  pal <- make_cie_pal(palette = palette, reverse = reverse)

  if (discrete) {
    ggplot2::discrete_scale("colour", palette = make_cie_pal_discrete, ...)
  } else {
...
#' @describeIn scale_colour_cie For fill scales
#'
#' @export
#'
scale_fill_cie <- function(
  ...
  if (discrete) {
    ggplot2::discrete_scale("fill", palette =  make_cie_pal_discrete, ...)
  } else {
...
#' Convenient functions to set CIE-appropriate palettes
#'
#' @param n Numeric. The number of levels in your colour scale. Minimum value is
#'   1, maximum is 7. Passed to \code{cie_pal}; see ?cie_pal for more
#'   information.
#'
...
# nocov start
cie_colour_manual <- function(n = 0,
                                  reverse = FALSE,
                                  discrete = TRUE,
                                  faded_level = 0,
                                  faded = FALSE,
  ...
  lifecycle::deprecate_warn(when = "1.0.0", what = "cie_colour_manual()",
                            details = "`cie_colour_manual` has been deprecated which means it will no longer be maintained or updated. Please use `scale_colour_cie` instead.")

  if (discrete) {
    return(
      ggplot2::scale_colour_manual(...,
                                   values = cietheme::cie_pal(n = n,
                                                                      reverse = reverse,
...
cie_fill_manual <- function(n = 0,
                                reverse = FALSE,
                                discrete = TRUE,
                                faded_level = 0,
                                faded = FALSE,
                                palette = "full", ...) {
  lifecycle::deprecate_warn(when = "1.0.0", what = "cie_fill_manual()",
                            details = "`cie_colour_manual` has been deprecated which means it will no longer be maintained or updated. Please use `scale_fill_cie` instead.")
