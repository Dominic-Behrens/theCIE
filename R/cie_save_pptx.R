...
#'
#' @description `cie_save_pptx()` creates a CIE-style Powerpoint
...
#' from which you called `cie_save_pptx()`. If you supply a list of plots,
...
#' function is usually best called by running `cie_save(..., save_pptx = T)`
#' or `cie_save_all()`, rather than calling it directly.
#'
...
#        caption = "Notes: notes go here. Source: source goes here.")
#
# cie_save_pptx(p2, filename = "test.pptx", rich_subtitle = TRUE)
#'
#' }
#' @export
#' @importFrom purrr walk walk2
cie_save_pptx <- function(filename,
                              p = ggplot2::last_plot(),
                              type = "fullslide",
                              rich_subtitle = FALSE,
                              png_dpi = 300,
...
add_graph_to_pptx <- function(p,
  ...
  for (slide in seq_len(num_slides)) {
    ...
    if (rich_subtitle) {
      ...
      body_font <- get_cie_font(font, "body")
...
pandoc_test <- function() {
  ...
  if (!rmarkdown::pandoc_available()) {
    stop("To use this function, you must install 'pandoc' on your system",
         " from pandoc.org. See the cietheme administrators",
         " if you need help.")
  }
  ...
  if (rmarkdown::pandoc_version() < 2.1) {
    stop("The version of 'pandoc' on your system is too old to use this",
         ...
         " See the cietheme administrators if you need help.")
  }
...
create_pptx_shell <- function(p,
  ...
  # Get path to appropriate PPTX template from `cietheme`
  template_filename <- system.file("extdata",
                                   chart_types_inc_deprecated$pptx_template[chart_types_inc_deprecated$type == type],
                                   package = "cietheme")
