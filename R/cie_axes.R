#' Ensure your y- and x-axis are CIE style guide-consistent and doesn't hang
...
#' @examples
#'
#' # Here's a basic chart in the CIE style:
#' library(ggplot2)
#'
...
#' p <- ggplot(mtcars, aes(x = wt, y = mpg)) +
#'      geom_point() +
#'      cie_y_continuous(expand_bottom = 0.015) +
#'      theme_cie()
#'
#' p
#'
...
#' @noRd
cie_label_hide_zero <- function(x) {
...
cie_y_continuous <- function(expand_bottom = 0, expand_top = 0.015, ...) {
  ...
  if (!"labels" %in% names(args)) {
    args$labels <- cie_label_hide_zero
  }
...
scale_y_continuous_cie <- function(expand_bottom = 0,
                                       expand_top = 0.015,
                                       ...) {

  cie_y_continuous(expand_bottom = expand_bottom,
...
cie_x_continuous <- function(expand_left = 0,
                                 expand_right = 0.015,
                                 ...) {
...
scale_x_continuous_cie <- function(expand_left = 0,
                                       expand_right = 0.015,
                                       ...) {

  cie_x_continuous(expand_left = expand_left,
