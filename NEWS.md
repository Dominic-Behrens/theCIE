...
Initial release. Fork of grattantheme 1.5, rebranded for the Centre for International Economics.

* All public symbols renamed: `grattan_*` &rarr; `cie_*`, `theme_grattan()` &rarr; `theme_cie()`, `scale_*_grattan()` &rarr; `scale_*_cie()`. No backwards-compatible aliases.
* Palette option `getOption("grattan_palette")` renamed to `getOption("cie_palette")`.
* S3 class `"grattan_theme"` renamed to `"cie_theme"`.
* Base colour hex values are placeholders pending CIE brand input; the palette machinery (`register_palette()`, `cie_palettes`, `cie_palette_order`, ~140 tint constants) is otherwise unchanged.
* PowerPoint templates and the logo image are inherited from grattantheme as placeholders.
* Font registration in `font_utils.R` retained; uses `getOption("cie_font_path")` if set, falling back to the previous Dropbox lookup.

---

# Inherited from grattantheme

The history below covers the upstream `grattantheme` package, which this package was forked from.
