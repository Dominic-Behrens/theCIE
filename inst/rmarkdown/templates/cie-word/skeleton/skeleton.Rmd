---
title: CIE Word Template for RMarkdown
subtitle: Live that all-RStudio lifestyle you've always dreamed of
author: Matt Cowgill
date: Created `r Sys.Date()`
output:
  redoc::redoc:
    reference_docx: rmd-cie-word-template.docx
    toc: yes
...
This is a template for an R Markdown document that you can Knit to a Word document. It will produce a document that is consistent with the basic elements of the CIE style. Note that the [redoc package](https://noamross.github.io/redoc/index.html) is required to generate Word documents that can be re-converted to R Markdown. If you don't have redoc, and don't want to install it for some reason, amend the output listed in the YAML header above from `redoc::redoc:` to `word_document:`.
