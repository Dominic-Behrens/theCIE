...
test_that("cie_save_pptx works in various configurations", {

  skip_on_cran()
  cie_save_pptx(p = p1, filename = "temp.pptx", type = "fullslide_43")
  ...
  cie_save_pptx(p = list(p1, p2), filename = "temp.pptx", type = "fullslide")
  expect_true(is_valid_pptx("temp_fullslide.pptx"))
  expect_equal(no_slides("temp_fullslide.pptx"), 2)
  unlink("temp", recursive = T, force = T)


  cie_save_pptx(p = list(p1, p2), filename = "multi_slide.pptx",
...
test_that("cie_save_pptx works when labels are present / absent", {

  skip_on_cran()
  # No labels
  cie_save_pptx(p = base_p1, filename = "temp.pptx")
  expect_true(is_valid_pptx("temp_fullslide.pptx"))
  expect_equal(no_slides("temp_fullslide.pptx"), 1)
  unlink("temp", recursive = T, force = T)

  # Just title
  cie_save_pptx(p = base_p1 + labs(title = "Some text"),
                    filename = "temp.pptx")
  expect_true(is_valid_pptx("temp_fullslide.pptx"))
  expect_equal(no_slides("temp_fullslide.pptx"), 1)
  unlink("temp", recursive = T, force = T)
  ...
  # Just subtitle
  cie_save_pptx(p = base_p1 + labs(subtitle = "Some text"),
                    filename = "temp.pptx")
  expect_true(is_valid_pptx("temp_fullslide.pptx"))
  expect_equal(no_slides("temp_fullslide.pptx"), 1)
  unlink("temp", recursive = T, force = T)
...
test_that("cie_save_pptx creates pptx with rich_subtitle = TRUE", {
