context("Check that theme_cie() and associated functions work")
...
p_scatter <- base_plot + theme_cie(chart_type = "scatter")

test_that("theme_cie() is a theme object", {
  expect_is(theme_cie(), "theme")
})

test_that("plot with theme_cie() is a ggplot2 object",{
  expect_is(p, "gg")
})
...
test_that("theme_cie() sends the right messages",{
  expect_message(base_plot +
                  theme_cie(chart_type = "scatter", flipped = TRUE))

  expect_warning(base_plot +
                   theme_cie(chart_type = "nah mate"))
...
test_that("theme_cie() wraps return with cie_theme class", {
  t <- theme_cie()
  expect_true(inherits(t, "cie_theme"))
  expect_true(inherits(t, "theme"))
  expect_false(is.null(attr(t, "cie_args")))
})
